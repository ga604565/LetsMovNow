...
class _CreateListingScreenState extends State<CreateListingScreen> {
  ...
  Future<void> _searchAddress() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Address search failed. Please try again.')),
        );
      }
    } finally {
  ...
  Future<void> _submit() async {
    ...
    if (_selectedUniName.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select a university from the list')),
      );
      return;
    }
    if (!_addressConfirmed) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please search and confirm your address first')),
      );
      return;
    }
    if (_images.isEmpty) {
    ...
    try {
      ...
      formData.fields.addAll([
        ...
        MapEntry('university',        _selectedUniName),
      ]);
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            const Text('Select your university first — city and state will be set automatically',
              ...
              children: [
                TextFormField(
                  ...
                  decoration: const InputDecoration(
                    ...
                    hintText: 'Start typing your university...',
                    prefixIcon: Icon(Icons.school_rounded, color: AppTheme.textSecondary, size: 20),
                  ),
                  validator: (_) => _selectedUniName.isEmpty ? 'Please select a university from the list' : null,
                ),
                if (_showUni && _uniSuggestions.isNotEmpty)
                  Container(
                    ...
                    child: Column(
                      children: _uniSuggestions.asMap().entries.map((entry) {
                        final u = entry.value;
                        final isLast = entry.key == _uniSuggestions.length - 1;
                        return InkWell(
                          onTap: () => _selectUni(u),
                          borderRadius: BorderRadius.circular(12),
                          child: Container(
            ...
            // City + State read-only (shown after selection)
            if (_city.isNotEmpty) ...[
              const SizedBox(height: 12),
              Row(children: [
                Expanded(flex: 2, child: _ReadOnlyField(label: 'City', value: _city)),
                const SizedBox(width: 12),
                Expanded(child: _ReadOnlyField(label: 'State', value: _state)),
              ]),
            ],

            // ── Step 2: Street Address (after uni selected) ──────
            if (_selectedUniName.isNotEmpty) ...[
              const SizedBox(height: 24),
              _SectionHeader('Address'),
              const SizedBox(height: 4),
              const Text('Enter the street number and name only — not shown publicly',
                  style: TextStyle(fontSize: 12, color: AppTheme.textSecondary)),
              const SizedBox(height: 10),
              TextFormField(
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      const Expanded(
                        child: Text('Address confirmed — this listing will appear on the map',
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(labelText: 'Title *', hintText: 'e.g. Cozy 1BR near UCF'),
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(labelText: 'Monthly Rent (\$) *', hintText: '950'),
              validator: (v) {
            ...
            TextFormField(
              ...
              maxLines: 4,
              decoration: const InputDecoration(labelText: 'Description', hintText: 'Describe the space, amenities, lease terms...'),
            ),
